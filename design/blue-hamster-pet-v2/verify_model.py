...
assert Path(bpy.data.filepath).resolve() == root / "blue-hamster-v2.blend"
assert bpy.context.scene.render.engine == "CYCLES"
assert bpy.context.scene.camera is not None
head = bpy.data.objects["HEAD POSE | reference clockwise tilt"]
assert abs(head.matrix_world.translation.z - 2.57) < .001
