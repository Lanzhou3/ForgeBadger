...
> 2026-05-15 补充：Models tab 改造为 Claude Code 优先的 Provider preset 目录：目录项覆盖 Anthropic API、Kimi、DeepSeek、Qwen、z.ai、OpenRouter 和 Ollama，并自带 endpoint/env/default models，用户主流程只需选择服务商并保存 API Key；Gateway provider/apply 契约支持 `claude` 与 `opencode`，Claude preview/apply 写入 `.claude/settings.local.json` 和 `ANTHROPIC_DEFAULT_*_MODEL` 默认模型环境变量，OpenCode 仍写入 `opencode.json`，Codex 继续保持 subscription-managed 且不进入 Provider API Key 配置流。Models 页面同时保留服务商搜索/有限高滚动，Copilot Provider/Model 选择在搜索过滤时保留当前选择，避免隐形提交。
> 2026-09-01 补充：上述 2026-08-31 统一绑定方案已被 ADR 0001（`docs/adr/0001-decouple-model-config-from-projects-and-sessions.md`）取代。Session 创建只收 `projectId` + `aiTool`，一律 `host_environment` 启动，不再注入 provider/model/credential 环境变量，`switch-model` 端点已删除。厂商配置收敛为 Model Center（`model_profiles`/`provider_credentials`）加显式 `POST /api/v1/cli-config/:adapter/apply-provider`（含 preview/rollback），按各 CLI 原生格式写入全局配置文件；凭据明文写入配置文件（`0600` 原子写 + AES-256-GCM 加密备份）。`/api/v1/cli-config/*` 不再 claim-gated，仅需 instance-admin。`model_provider_bindings` 表与 sessions `launch_*` 列仅为迁移连续性保留，live 代码不再读写。
...
3. **tmux 会话生命周期** — 命名规范 `fb-{user_short}-{session_short}`，Gateway 重启不影响
...
| API | `GET/POST /api/v1/sessions`、`POST /api/v1/sessions/:id/start`、`POST /api/v1/sessions/:id/stop` |
...
| `FORGEBADGER_TMUX_PREFIX` | ❌ | fb- | tmux 前缀 |
