...
> 2026-05-11 补充：Platform AI Copilot 第一版独立架构规格已确认，见 `docs/superpowers/specs/2026-05-11-platform-ai-copilot-design.md`；实施计划见 `docs/superpowers/plans/2026-05-11-platform-ai-copilot.md`。第一版定位为 provider-backed、read-heavy、approval-gated 的平台操作助手，不开放自主开发、自动 tmux 输入、任意 shell 或 Codex app-server prompt/turn 输入；OpenClaw 参考点已吸收到 provider seam、memory recall、session visibility 和 approval canonical payload 约束中。当前已完成 provider-backed text runs、Web Copilot 页面、显式 Provider/Model 选择、read-only tool registry（含 project/session detail 与 diagnostics summary）、approval-gated pending actions、run/tool/pending-action lifecycle 与 cancel 系统审计、provider HTTP failure 分类与本地化提示、in-flight model cancel/timeout 控制、waiting-for-approval Web cancel 控件、prompt persistence/provider-request redaction、tool-output size/sensitive-output fail-closed safety、显式 tenant-scoped Copilot memory（SQLite FTS、memory_search/memory_get/propose_memory_write）、user-facing Copilot source 的短时 active recall、per-user run concurrency guard 和 diagnostics capability/count 元数据；memory 不做静默长期写入、终端 transcript 索引或 embeddings。
