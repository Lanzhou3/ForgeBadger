...
> 2026-05-17 补充：Feishu Project Manager Copilot 规格与实施计划已确认，首个实现切片已完成 Feishu CLI 只读发现、`/api/v1/integrations/feishu/status`、Settings 页飞书集成诊断卡片、诊断导出中的安全飞书能力摘要和聚焦 Playwright 覆盖。当前仍不开放飞书入站控制、飞书写操作、未批准终端输入或无人值守开发循环；后续按计划进入 Feishu 集成配置持久化与审批受控的 outbound action。
