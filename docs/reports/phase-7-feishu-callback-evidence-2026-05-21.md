...
- Do not record raw auth config, app secrets, verification tokens, event encryption keys, raw signatures, JWTs, callback bodies, or private message text.
...
| Real console URL verification | Pending | Configure Feishu developer-console event subscription callback URL to `https://<public-host>/api/v1/integrations/feishu/webhook/<publicWebhookId>` and run URL verification | Requires Gateway reachable through public HTTPS and Feishu developer-console access | Not attempted in this preflight entry. | This report | Pending real developer-console action. | owner: OpenForge operator | Run Feishu console URL verification after public URL and live webhook config are available. |
| Optional real message event | Pending | Subscribe to `im.message.receive_v1`, send one message in an allowed chat, and inspect sanitized Gateway audit/log status | Requires URL verification pass, allowed chat id, and Feishu open_id mapping | Not attempted in this preflight entry. | This report | Pending URL verification and live event setup. | owner: OpenForge operator | Attempt only after URL verification and policy mapping are configured. |
