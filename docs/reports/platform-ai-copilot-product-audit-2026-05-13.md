...
| Explicit Provider/Model selection in Web | `packages/web/e2e/copilot.spec.ts` covers selected provider/model run creation and metadata display; `packages/web/src/lib/copilot.test.ts` covers selectable providers and credential-ready filtering. | Covered by Web unit and E2E tests |
...
| Approval-gated actions only | `docs/API.md` documents prepare tools and canonical stored pending-action approval; route/Web tests cover approve, reject, duplicate submission prevention, waiting-for-approval cancel, and audit rows. | Covered |
...
| Explicit tenant-scoped memory | `docs/API.md` documents explicit memory state; `packages/gateway/test/copilot-memory-repository.test.ts` covers tenant scoping and SQLite FTS; route/tool tests cover memory search/get/propose/write flows. | Covered |
...
| Release/PR state | PR #2 (`feat: add release gates and platform copilot`) is open, non-draft, and verified with `mergeStateStatus=CLEAN`; all four checks were successful: Workspace Static Checks/Tests/Builds, Codex Background Task Gates, NPM Package Build/Verify/Smoke, and Environment-Gated Release Notes. | Covered as release gate evidence |
