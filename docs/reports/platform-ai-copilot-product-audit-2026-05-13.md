...
| Approval-gated actions only | `docs/API.md` documents prepare tools and canonical stored pending-action approval; route/Web tests cover approve, reject, duplicate submission prevention, waiting-for-approval cancel, audit rows, unknown stored action rejection, and invalid troubleshooting-step approval rejection. | Covered |
