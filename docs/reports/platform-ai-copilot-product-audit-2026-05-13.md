...
| Adapter discovery recovery action | `openforge.propose_adapter_refresh` creates an approval-gated pending action; approval reruns adapter discovery and returns fresh availability/launch-readiness metadata without starting CLI sessions. `packages/gateway/test/copilot-tools.test.ts`, `packages/gateway/test/copilot-routes.test.ts`, and `packages/web/src/lib/copilot.test.ts` cover the tool contract, approval result, and Web labels/summaries. | Covered |
