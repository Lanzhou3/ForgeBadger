...
Session Server sessions (POSIX PTY on macOS/Linux/WSL, ConPTY on
native Windows):
...
- Session Server-backed AI CLI sessions;
...
| Physical Windows/WSL terminal | Caveat until a physical native Windows ConPTY + Session Server lifecycle and/or real WSL POSIX PTY lifecycle completes the checklist. | Run `docs/TRIAL-CHECKLIST.md` Windows/WSL section and attach bounded, redacted terminal-runtime evidence. Ubuntu/Linux CI does not clear this gate. |
...
- independent Session Server as the terminal persistence layer, with node-pty on
