...
import { getCliBrand } from "./cli-brand";
...
describe("getCliBrand", () => {
  it("returns the brand for each supported CLI", () => {
    ...
    expect(getCliBrand("kimi")).toMatchObject({ label: "Kimi Code", color: "#8b5cf6" });
    expect(getCliBrand("opencode")).toMatchObject({ label: "OpenCode", color: "#22c55e" });
  });
