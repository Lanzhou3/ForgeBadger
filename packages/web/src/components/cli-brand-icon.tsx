...
/**
 * Inline SVG path data for each CLI's official brand mark. Marks are inlined
 * (instead of loaded via <img>) so monochrome marks can inherit `currentColor`
 * and adapt to their surrounding text color.
 *
 * Sources — official vendor marks via the LobeHub icon registry
 * (`@lobehub/icons-static-svg`, which tracks vendor branding); raw copies are
 * kept at `public/brand/cli/*.svg`:
 * - Claude:  https://unpkg.com/@lobehub/icons-static-svg/icons/claude-color.svg
 *            (Anthropic Claude starburst, terracotta #D97757)
 * - Codex:   https://unpkg.com/@lobehub/icons-static-svg/icons/codex.svg
 *            (OpenAI Codex knot, intentionally monochrome)
 ...
 */
const CLI_ICON_MARKS: Record<CliBrandId, ReactNode> = {
  claude: (
    <path
...
const CURRENT_COLOR_MARKS: ReadonlySet<string> = new Set(["codex", "opencode"]);

export function CliBrandIcon({ aiTool, className }: Props) {
  const brand = getCliBrand(aiTool);
  const mark = brand.id === "unknown" ? null : CLI_ICON_MARKS[brand.id];
  if (!mark) {
    return null;
  }
  return (
