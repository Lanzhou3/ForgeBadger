# @forgebadger/dsh-bridge

ForgeBadger copilot 的 dsh（DeepSeek Harness）内核桥接包：平台能力 Cordis 插件 +
resume 感知 JSON-RPC server + per-user runtime launcher。对应
`.planning/dsh-integration/PLAN.md` 的 M1。

## 组成

- `src/plugin.ts`（`@forgebadger/dsh-bridge/plugin`）— forgebadger-bridge 插件，把平台能力
  ...
  走 M3 审批桥。
- `src/server.ts`（`@forgebadger/dsh-bridge/server`）— resume 感知 SDK JSON-RPC server。
...
- `src/launcher.ts`（bin `forgebadger-dsh-bridge`）— per-user runtime 启动器，供 M2
...
- `templates/cordis.yml` — per-user 组合模板（MiniMax 经 `dsh-llm-pi-ai` anthropic
  ...
  配置渲染裁剪副本（`# @forgebadger-feature` 标记块，如 compaction/subagents），写到
...
| 变量 | 必填 | 说明 |
|------|------|------|
| `FORGEBADGER_GATEWAY_URL` | 否（默认 `http://127.0.0.1:48731`） | Gateway base URL |
| `FORGEBADGER_COPILOT_BRIDGE_TOKEN` | 是 | 内部 API bearer token |
| `FORGEBADGER_USER_ID` | 是 | 租户 id（`X-ForgeBadger-User-Id`） |
| `FORGEBADGER_BRIDGE_TIMEOUT_MS` | 否（默认 15000） | 单次内部 API 超时 |
...
Base `{FORGEBADGER_GATEWAY_URL}/api/internal/v1/copilot-bridge`，
`Authorization: Bearer $FORGEBADGER_COPILOT_BRIDGE_TOKEN` + `X-ForgeBadger-User-Id`，
...
`packages/dsh-bridge` 是依赖本地 DeepSeek Harness 检出的实验集成，已从默认
pnpm workspace 排除。普通贡献者执行根目录 `pnpm install`、递归构建和测试时不需要
该外部仓库，也不会生成指向维护者目录的断链。开发此包前，先按下节准备 Harness，
再以独立项目模式安装：

```bash
pnpm --ignore-workspace --dir packages/dsh-bridge install
pnpm --ignore-workspace --dir packages/dsh-bridge build
FORGEBADGER_COPILOT_BRIDGE_TOKEN=... FORGEBADGER_USER_ID=... MINIMAX_API_KEY=... \
...
```bash
pnpm --ignore-workspace --dir packages/dsh-bridge test  # node:test + tsx，stub HTTP server
