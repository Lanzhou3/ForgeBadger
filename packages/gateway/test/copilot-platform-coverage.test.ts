...
function fixture(){
  ...
  const grant=new PlatformActions({db,userId:user.id},createPlatformCommands()).createGrant({name:'scope',projectIds:[project.id],capabilities:['pm.work_item.metadata'],expiresAt:Date.now()+100000,maxActions:10});
  ...
  return {db,user,other,project,outside,foreign,session,pm,item,grant,registry,manager,ctx,call,takenOver:()=>takenOver};
}
it('registers bounded schemas and enforces owner/runtime/source visibility',()=>{
  const f=fixture();try{
    ...
    for(const options of [{hasSessionManager:true,grantBound:true},{hasSessionManager:true,scheduled:true},{hasSessionManager:true,reactive:true},{hasSessionManager:false},{hasSessionManager:true,isToolDisabled:(name:string)=>name==='takeover_session'}]){
...
it('reads complete goal/task/management/declared ledger within tenant and Grant scope',async()=>{
  const f=fixture();try{
    ...
    for(const name of names.slice(0,4)){
      ...
      assert.equal((await f.call(name,input,{...f.ctx,grantId:f.grant.id})).ok,true);
      for(const projectId of [f.outside.id,f.foreign.id])assert.equal((await f.call(name,{...input,projectId},{...f.ctx,grantId:f.grant.id})).ok,false);
    ...
    assert.equal((await f.call('pm_get_goal',{projectId:f.project.id},{...f.ctx,grantId:f.grant.id})).ok,false);
  }finally{f.db.close();}
});
...
async function takeoverRun(f:ReturnType<typeof fixture>,options:{source?:'user'|'scheduled'|'reactive';grantId?:string;disabled?:()=>boolean}={}){
  ...
  const runId=await orchestrator.runTurn({userId:f.user.id,conversationId:conversation.id,userText:'Take over this session',...(options.source?{source:options.source}:{}),...(options.grantId?{grantId:options.grantId}:{})});
...
it('forged Grant/scheduled/reactive/disabled takeover never creates approval or effects',async()=>{
  const f=fixture();try{
    for(const options of [{grantId:f.grant.id},{source:'scheduled' as const},{source:'reactive' as const},{disabled:()=>true}]){
    ...
    for(const context of [{...f.ctx,grantId:f.grant.id},{...f.ctx,source:'scheduled'},{...f.ctx,source:'reactive'}])assert.equal((await f.call('takeover_session',{sessionId:f.session.id},context)).ok,false);
  }finally{f.db.close();}
});
...
it('rejects cross-project writer and stale resources; unexpected writer errors fail closed',async()=>{
  const f=fixture();try{
    ...
    assert.equal((await f.call('get_session_writer',{sessionId:outsideSession.id},{...f.ctx,grantId:f.grant.id})).ok,false);
