...
function fixture() {
  ...
  const runId=ledger.admit({userId:user.id,conversationId:conversation.id,userText:'inspect'},5);
  ...
  const context:AgentToolContext={db,userId:user.id,masterKey:'test',conversationId:conversation.id,runId,availableToolNames:['list_projects','read_tool_result'],checkExecutionAuthority:()=>true};
  return {db,user,ledger,conversation,runId,step,tool,messageId,context,content,project};
}
...
it('revalidates channel authority before readback',async()=>{
  const f=fixture();try{
...
it('revalidates source resource scope and rejects nested or external readbacks',async()=>{
  const f=fixture();try{
    const outsider=new UserRepository(f.db).create('outsider@test.dev','hash');
    const outside=new ProjectRepository(f.db,outsider.id).create({name:'outside',path:'/tmp/readback-outside',aiTool:''});
    f.db.prepare('UPDATE copilot_run_steps SET input_json=? WHERE id=?').run(JSON.stringify({projectId:outside.id}),f.step.id);
    assert.equal((await executeAgentTool(f.tool,{messageId:f.messageId},f.context)).ok,false);
    f.db.prepare('UPDATE copilot_run_steps SET input_json=? WHERE id=?').run(JSON.stringify({projectId:f.project.id}),f.step.id);
    assert.equal((await executeAgentTool(f.tool,{messageId:f.messageId},f.context)).ok,true);
