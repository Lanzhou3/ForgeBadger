...
interface ContentBlock { native: Record<string, unknown>; type: string; text: string; tool?: { id: string; name: string; arguments: string }; initialInput?: Record<string, unknown>; closed: boolean }

function parseBlock(value: unknown): ContentBlock {
  const block = record(value);
  if (block.type === "text") return { native: { ...block }, type: "text", text: optionalText(block.text), closed: false };
  if (block.type === "thinking") return { native: { ...block }, type: "thinking", text: optionalText(block.thinking), closed: false };
  if (block.type === "redacted_thinking") return { native: { ...block }, type: "redacted_thinking", text: "", closed: false };
  if (block.type !== "tool_use") invalidResponse("unsupported content block");
  return { native: { ...block }, type: "tool_use", text: "", closed: false, initialInput: record(block.input),
...
function finishBlock(block: ContentBlock, completion: LlmCompletion): void {
  ...
  block.closed = true;
  if (block.tool) {
    const tool = checkedTool({ ...block.tool, arguments: block.tool.arguments || JSON.stringify(block.initialInput) });
    completion.toolCalls.push(tool);
    block.native.input = parseJson(tool.arguments);
  }
}
...
function jsonCompletion(value: unknown): LlmCompletion {
  ...
  const completion: LlmCompletion = { message: "", thinking: "", toolCalls: [], replay: { format: "anthropic", blocks: [] },
  ...
  for (const value of data.content) {
    ...
    completion.replay!.blocks!.push(block.native);
  }
...
function applyBlockDelta(data: Record<string, unknown>, block: ContentBlock, completion: LlmCompletion, emit: LlmEmit): void {
  ...
  if (delta.type === "signature_delta" && block.type === "thinking") {
    block.native.signature = optionalText(block.native.signature) + optionalText(delta.signature); return;
  }
  if (delta.type === "text_delta" && block.type === "text") { block.text = optionalText(delta.text); block.native.text = optionalText(block.native.text) + block.text; addText(block, completion, emit); return; }
  if (delta.type === "thinking_delta" && block.type === "thinking") { block.text = optionalText(delta.thinking); block.native.thinking = optionalText(block.native.thinking) + block.text; addText(block, completion, emit); return; }
...
async function sseCompletion(response: Response, emit: LlmEmit, signal: AbortSignal): Promise<LlmCompletion> {
  ...
  completion.replay = { format: "anthropic", blocks: [...blocks.entries()].sort(([a], [b]) => a - b).map(([, b]) => b.native) };
