...
export function projectActionReceipt(receipt:ActionReceipt):string {
 if(receipt.outcome==='confirmed' && receipt.result && typeof receipt.result==='object'
  && 'executionStatus' in receipt.result && receipt.result.executionStatus==='incomplete') {
  return `Task not dispatched; completed preparation stages are recorded. Resume only the remaining not-sent stage with a new authorized action. ${JSON.stringify(redactAgentValue({platformIntentId:receipt.intentId,receiptOutcome:receipt.outcome,result:receipt.result}))}`;
 }
