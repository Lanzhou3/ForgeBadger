import { redactTelegramError } from "./telegram-error-redaction.js";
import type {
  TelegramPollingCallbacks,
  TelegramPollingHandle,
  TelegramPollingHandlers
} from "./telegram-polling-client.js";

export interface TelegramSupervisorAccount {
  userId: string;
  accountId: string;
  botToken: string;
...
interface AccountSource {
  listEnabled(): TelegramSupervisorAccount[] | Promise<TelegramSupervisorAccount[]>;
  get(userId: string): TelegramSupervisorAccount | undefined | Promise<TelegramSupervisorAccount | undefined>;
  updateHealth(userId: string, health: TelegramConnectionHealth): void | Promise<void>;
  updateBotUsername?(userId: string, botUsername: string | null): void | Promise<void>;
}

interface SupervisorPollingFactory {
  createPollingClient(
    config: TelegramSupervisorAccount,
...
interface RuntimeEntry {
  account: TelegramSupervisorAccount;
  client?: TelegramPollingHandle;
  generation: number;
  failed: boolean;
  retryTimer?: unknown;
}

export class TelegramConnectionSupervisor {
  private readonly runtimes = new Map<string, RuntimeEntry>();
  private readonly health = new Map<string, TelegramConnectionHealth>();
  private readonly healthWrites = new Map<string, Promise<void>>();
  private readonly operations = new Map<string, number>();
  private readonly handlers = new Map<string, TelegramPollingHandlers>();
  ...
  constructor(private readonly dependencies: {
    ...
    createPollingClient: SupervisorPollingFactory["createPollingClient"];
    createHandlers?: (account: TelegramSupervisorAccount) => TelegramPollingHandlers;
  ...
  registerHandlers(userId: string, handlers: TelegramPollingHandlers): void {
  ...
  private async refreshAccount(userId: string, retryAttempt: number): Promise<void> {
    ...
    let account: TelegramSupervisorAccount | undefined;
    try { account = await this.dependencies.accounts.get(userId); }
      ...
      if (this.isOperationCurrent(userId, generation, lifecycle)) {
        ...
        this.publishHealth(userId, { ...this.getHealth(userId), state: "unhealthy", lastErrorMessage: "TELEGRAM_ACCOUNT_READ_FAILED" });
      }
      return;
    }
  ...
  private connect(account: TelegramSupervisorAccount, generation: number, retryAttempt: number): void {
    ...
    try {
      entry.client = this.dependencies.createPollingClient(account, callbacks, this.guardHandlers(entry));
  ...
  private createCallbacks(entry: RuntimeEntry, retryAttempt: number): TelegramPollingCallbacks {
    const publish = (health: () => TelegramConnectionHealth): void => {
    ...
    return {
      ...
      onIdentity: (identity) => {
        if (this.isActive(entry)) {
          void this.dependencies.accounts.updateBotUsername?.(entry.account.userId, identity.username);
        }
      }
    };
  }
  ...
  private handleTerminalError(entry: RuntimeEntry, retryAttempt: number, error: Error): void {
    ...
    this.publishHealth(userId, {
      ...
      lastErrorMessage: redactTelegramError(error)
    });
    entry.retryTimer = this.timers.set(() => {
      if (!this.isOwned(entry)) return;
      entry.retryTimer = undefined;
      void this.refreshAccount(userId, nextAttempt).catch(() => this.reportFailure("TELEGRAM_RETRY_FAILED"));
  ...
  private closeEntry(entry: RuntimeEntry): void {
    ...
    } catch { this.reportFailure("TELEGRAM_TIMER_CLEAR_FAILED"); }
    entry.retryTimer = undefined;
    try { entry.client?.close(true); }
    catch { this.reportFailure("TELEGRAM_CLIENT_CLOSE_FAILED"); }
  }

  private publishHealth(userId: string, health: TelegramConnectionHealth): void {
    ...
    }).catch(() => this.reportFailure("TELEGRAM_HEALTH_WRITE_FAILED"));
  ...
  private reportFailure(code: string): void {
    console.error("[telegram-supervisor] lifecycle operation failed", { code });
  }
...
function disabledHealth(account: TelegramSupervisorAccount | undefined): TelegramConnectionHealth {
  return {
    state: "disabled",
...
function connectedHealth(account: TelegramSupervisorAccount): TelegramConnectionHealth {
  return {
    state: "connected",
