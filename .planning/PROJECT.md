...
triage, the external gate registry now preserves that route, support/open-source entrypoints now keep both audit routes visible, root/localized README trial entrypoints now keep both feedback collection paths visible, completed-feedback audits now require Copilot evidence fields before triage, and maintainers can scan non-tracker GitHub feedback candidates in one read-only pass. The remaining work is real first-user trial packet collection and
maintainer triage.
...
- [x] v1.5 external gate validation preserves the GitHub issue-form feedback
  audit rerun path for `FIRST-USER-FEEDBACK`.
- [x] v1.5 public and support entrypoint docs preserve both feedback audit
...
- [x] v1.5 external evidence registry protects the issue-form audit rerun path
  alongside the Markdown packet audit path.
- [x] v1.5 trial intake validation covers open-source readiness and support
...
- Phase 33 updated the `FIRST-USER-FEEDBACK` external gate rerun path and
  `pnpm evidence:gates-validate` so both Markdown packet audit and GitHub
  issue-form audit commands are protected. External gate states remain
  unchanged.
- Phase 34 extended `pnpm trial:intake-validate` to guard
  `docs/OPEN-SOURCE-READINESS.md` and `docs/SUPPORT-DIAGNOSTICS.md` so both
  public/support entrypoints preserve the Markdown packet audit and GitHub
  issue-form audit routes before maintainer triage.
- Phase 35 extended `pnpm trial:intake-validate` to guard `README.md`,
...
- Phase 37 added `pnpm trial:feedback-issues-audit`, a read-only GitHub
  `trial-feedback` candidate scan that skips route trackers, audits
  non-tracker candidates, and reports ready/blocked issue numbers without
  clearing gates.
