...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 05-01-01 | 05-01 | 1 | REM-01 | REM-T01..REM-T10 | Remote architecture package covers execution targets, remote agent, host keys, path safety, diagnostics, rollback, and staged implementation without runtime code. | docs/static | Task 1 fail-closed checks verify every required heading, link, threat id, failure code, and Codex boundary string. | present | pending |
| 05-01-02 | 05-01 | 1 | REM-02 | REM-T08, REM-T10 | Hosted/cloud/billing/telemetry/marketplace scope remains deferred and does not enter local-first runtime paths. | static/evidence | Verification report records hosted/cloud scope scan results and classifies every match as deferred/boundary text or scope leak. | present | pending |
| 05-01-03 | 05-01 | 1 | COD-01 | REM-T09 | Codex app-server `/turn` remains disabled by default and Web prompt/turn input is not exposed. | evidence/static | Verification report records exact result or caveat for `pnpm --dir packages/gateway test test/codex-app-server-routes.test.ts` and `pnpm --dir packages/web exec playwright test e2e/codex-app-server.spec.ts --project=chromium --reporter=line` | present | pending |
